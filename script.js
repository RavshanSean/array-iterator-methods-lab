...
firstLast = people.map((person, i) => {
    const splitPerson = person.split(', ');
    const rearranged = [splitPerson[1], splitPerson[0]]
...
let isAdultPresent = inventors.some(inventor => (inventor.passed - inventor.year) >= 18)
...
let isEveryone19OrOlder = devs.every(devs => (2024 - devs.year) >= 19)
